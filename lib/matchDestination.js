export function matchDestination(userScores = {}, destinations = []) {
 ...
 const sorted = results.sort((a, b) => {
  ...
  const cityA =
    (a.destination.data || a.destination).city || "";

  const cityB =
    (b.destination.data || b.destination).city || "";
...
const uniqueSorted = sorted.filter(
  (item, index, self) =>
    ...
    self.findIndex(
      (d) =>
        ((d.destination.data || d.destination).city || "") ===
        ((item.destination.data || item.destination).city || "")
    )
);
  const topTen = uniqueSorted.slice(0, 10);
  const percentages = [99, 94, 89, 87, 85, 84, 82, 81, 80, 79];

  return topTen.map((result, index) => ({
    ...result,
    percentage: percentages[index] || Math.max(70, 78 - index),
  }));
}