...
import { getDestinationBadge } from "@/lib/destinationBadge";
...
adventure: 10,
  ...
  {
    id: 9,
      question: "Where could you spend hours without getting bored?",
    answers: [
  ...
  {
    id: 16,
    question: "What personality would your ideal destination have?",
    answers: [
...
function ResultCard({ match, onSeeDetails }) {
  ...
  const badge = getDestinationBadge(data);
...
setUserPersonality(getTravelPersonality(finalScores));
...
if (loading) {
  return (
   ...
   <p>✓ Building your travel profile</p>
  <p>✓ Finding ideal places</p>
  <p>✓ Finding emotional matches</p>
  <p>✓ Comparing destination DNA</p>
  <p>✓ Ranking your best matches</p>
...
<div className="relative z-10 max-w-7xl mx-auto -mt-12">
  ...
  Your Travel Matches
          ...
          <p className="text-center mt-2 text-white/60 mb-10">
...
{result.length > 3 && (
  ...
  >
    ...
    <div className="flex items-start justify-between mb-3">
      <h3 className="flex-1 text-lg font-semibold text-white leading-tight line-clamp-2">
        {data.city}, {data.country}
      </h3>

      {/* Desktop percentage */}
      <div className="hidden md:block shrink-0 text-2xl font-bold text-[#8f84ff] ml-4">
        {match.percentage}%
      </div>
    </div>
    ...
    <div className="flex items-start gap-4">
      ...
      {/* Image */}
    <div className="relative w-28 h-28 mt-2 md:mt-0 rounded-2xl overflow-hidden flex-shrink-0 bg-white/5">
<Image
  src={data.image}
  alt={data.city}
  fill
...
/>
  ...
  >
    {match.percentage}%
      ...
      {/* Description */}
     <p className="flex-1 text-sm text-white/60 italic leading-relaxed">
        {emotionalHook}
      </p>
...
<div className="relative z-10 mt-8 max-w-3xl mx-auto pb-24">
  <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl py-4 px-5 md:p-6">
...
{userPersonality && (
  <div className="inline-flex items-center gap-2 rounded-full border border-[#6d5dfc]/30 bg-[#6d5dfc]/15 px-4 py-2 text-sm font-semibold text-[#c8c2ff] mb-4">
    <span>{userPersonality.icon}</span>
    <span>{userPersonality.label}</span>
  </div>
)}
...
{shareOpen && (
  ...
  >
    <div
      onClick={(e) => e.stopPropagation()}
      className="relative z-10 w-full md:max-w-2xl max-w-3xl rounded-3xl border border-white/15 bg-white/10 backdrop-blur-2xl p-10 text-center shadow-[0_30px_100px_rgba(0,0,0,0.45)]"
    >
...
<footer className="relative z-10 border-t border-white/10">
 ...
 return (
  <main className="min-h-screen bg-[#070b16] text-white overflow-hidden relative flex justify-center items-start pt-6 md:items-center md:py-10 px-6">
    <div
   ...
   <div className="relative md:-mt-6 z-10 w-full max-w-3xl md:max-w-2xl rounded-3xl border border-white/15 bg-white/10 backdrop-blur-2xl p-6 md:px-10 md:pt-10 md:pb-5 shadow-[0_30px_100px_rgba(0,0,0,0.45)]">
      ...
      <h1 className="text-3xl font-bold mb-8 text-center">
        ...
        <div className="flex flex-col gap-4">
          {questions[current].answers.map((a, i) => (
            ...
            className={`py-2 px-5 text-[15px] md:py-3.5 md:text-[17px] rounded-xl border text-left transition ${
                selectedAnswer === a
        ...
        <div className="flex justify-between items-center mt-10">
          <button
            onClick={goPrevious}
            disabled={current === 0}
            className={`min-w-[130px] py-4 rounded-xl font-semibold transition ${
              current === 0
                ? "bg-white/10 text-white/30 cursor-not-allowed"
  ...
  className="min-w-[130px] py-4 rounded-xl text-white font-semibold bg-gradient-to-br from-[#4f7cff] to-[#6d5dfc] shadow-lg hover:scale-105 transition disabled:opacity-40 disabled:cursor-not-allowed">
